...
public class F_stringInReverse {
	...
	/*Prefer: 
     *first i value(equated to .length) SHOULD REPRESENT the actual index of the last element 
	 *(without -1 it represents an out of bound index)
	...
	// As soon as the compiler reads: "phrase.charAt"  then,
	// it automatically assigns index values to each character of the string named 'phrase',
	// from 0 to max. Now you're asking for CharAt(max plus 1) which can not be found/ out of bound by a notch
	...
	public void reveseB() {
		...
		String phrase= "Print me in reverse";
		String reversed= "";
		// below, the unpreferred way of doing it, since in the .charAt() method one has to included i=i-1
	 ...
	 //first: print out, in reverse, the elements present in the below array (fixed-length data structure)
	   @Test(enabled= true)
